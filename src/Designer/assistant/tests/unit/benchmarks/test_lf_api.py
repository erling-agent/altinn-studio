...
def _row(session_id, trace_id, name="Assistant Agent Workflow"):
...
class TestFindTraceForSession:
    def test_reads_the_trace_id_off_the_matching_root_span(self):
        ...
        assert api.find_trace_for_session("sess-b", "Assistant Agent Workflow", "2026-01-01") == "trace-b"

    def test_queries_the_observations_endpoint(self):
        api = _Api([{"data": [_row("sess-a", "trace-a")], "meta": {}}])
        api.find_trace_for_session("sess-a", "Assistant Agent Workflow", "2026-01-01")
    ...
    def test_asks_for_metadata_and_a_bounded_window(self):
        ...
        assert params["name"] == "Assistant Agent Workflow"
    ...
    def test_follows_the_cursor_to_the_next_page(self):
        ...
        assert api.find_trace_for_session("sess-a", "Assistant Agent Workflow", "2026-01-01") == "trace-a"
        assert api.calls[1][1]["cursor"] == "CURSOR1"

    def test_no_match_returns_none(self):
        api = _Api([{"data": [_row("other", "trace-x")], "meta": {}}])

        assert api.find_trace_for_session("missing", "Assistant Agent Workflow", "2026-01-01") is None
